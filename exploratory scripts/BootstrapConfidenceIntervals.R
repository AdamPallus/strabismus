...
bootstrap.stat<- function(samp,z,chosenParam='slowverg'){
  
  #convert to data.table for faster processing
  z <- as.data.table(z)
  setkey(z, chosenParam) #like group_by but in data.tables jargon
...
neurons<- unique(xxSLOW$neuron)

ptb %>%
  filter(neuron %in% neurons) %>%
  # filter(neuron=='Kopachuck-101') %>%
...
for (i in seq_along(neurons)){
  cis[[i]]<-bca.ci(filter(xxSLOW,neuron==neurons[i]),filter(ptb,neuron==neurons[i]))
}
...
ptb %>%
  group_by(neuron) %>%
  summarize(n=length(unique(slowverg)))->
  ptb.n
